...
class Seq2Seq(nn.Module):
    ...
    def forward(self,x,y,teacher_forcing_ratio):
        hidden_last_layers,hidden_last_steps = self.encoder(x)
        if not self._train: teacher_forcing_ratio = 0.0

        outputs = self.decoder(hidden_last_steps,y,teacher_forcing_ratio)
...
class Decoder(nn.Module):
    ...
    def forward(self,hidden,y,teacher_forcing_ratio):
        layers,batch_size,hidden_dim = hidden.shape
        ...
        for t in range(self.output_len):
            if t == 0:
                input = torch.zeros((batch_size,1,self.output_dim)).float().to(DEVICE)
            else:
                tf = random.random() < teacher_forcing_ratio
                if tf: input = y[:,t-1,:].unsqueeze(1) #teacher forcing
